...
def compute_features_from_generator(generator, n_total, clip_duration, output_file, device="cpu", ncpu=1):
    ...
    # WINDOWS FIX: this write-mmap (fp) keeps an OS lock on output_file. On
    # Windows trim_mmap() then can't delete the file (WinError 32). Close the
    # underlying mmap and drop fp BEFORE trimming.
    _mm = getattr(fp, "_mmap", None)
    if _mm is not None:
        try:
            _mm.close()
        except Exception:
