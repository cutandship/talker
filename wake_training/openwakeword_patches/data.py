...
def trim_mmap(mmap_path):
    ...
    # WINDOWS FIX: a numpy memmap keeps an OS-level lock on its file through an
    # internal mmap object — gc alone does NOT release it, so os.remove() raised
    # WinError 32 and killed training. Close the underlying mmap explicitly on
    # BOTH arrays, then remove with a short backoff for good measure.
    import gc as _gc
    import time as _t
    try:
        mmap_file2.flush()
    ...
    for _attempt in range(20):
        ...
        os.remove(mmap_path)  # final attempt: let it raise if truly stuck
