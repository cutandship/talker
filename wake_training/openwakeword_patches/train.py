...
#
# NOTE: This is a MODIFIED copy of openWakeWord's train.py
# (https://github.com/dscripka/openWakeWord), changed by Cut & Ship for the
# Talker wake-word pipeline. See ./LICENSE, ./NOTICE and ./README.md.
...
if __name__ == '__main__':
    ...
    if args.train_model is True:
        ...
        if n_cpus is None:
            ...
            n_cpus = n_cpus//2
        # WINDOWS FIX: num_workers>0 uses spawn-multiprocessing here, which must
        # pickle the IterDataset (it wraps a generator holding lambdas) — that
        # raises PicklingError/EOFError on Windows and kills training. Run the
        # loader in the main process (num_workers=0); the generator is already
        # mmap-backed and fast enough.
        ...
        # Convert the model from onnx to tflite format. OPTIONAL — needs onnx_tf
        # + tensorflow, which we intentionally don't install (Talker uses the
        # .onnx only). Never let a missing tflite converter fail the whole run
        # AFTER the .onnx was already saved successfully.
        if args.convert_to_tflite:
            try:
                ...
                logging.warning(f"tflite conversion skipped ({_e}); .onnx is ready")
